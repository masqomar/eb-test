...
class GradeRepository extends BaseRepository implements GradeInterface
{
    ...
    {
        return $this->model->with(['category', 'exam', 'gradeDetail', 'gradeDetail.valueCategory', 'exam.questionTitle.question', 'exam.questionTitle'])->find($id);
    }

    public function getRankingByExam($examId, $limit = 10)
    {
        return $this->model->where(['exam_id' => $examId, 'is_finished' => 1])->with(['user.student', 'user.student.province', 'user.student.city'])->orderBy('grade_old', 'DESC')->paginate($limit);
    }
}
